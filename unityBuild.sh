...
get_unity_job_worker_count() {
    if [[ -n "${UNITY_JOB_WORKER_COUNT:-}" ]]; then
        echo "$UNITY_JOB_WORKER_COUNT"
        return 0
    fi
...
get_unity_gc_helper_count() {
    if [[ -n "${UNITY_GC_HELPER_COUNT:-}" ]]; then
        echo "$UNITY_GC_HELPER_COUNT"
        return 0
    fi
...
UNITY_RUNTIME_ARGS_RESULT=()

is_unity_accelerator_available() {
    local endpoint="$1"
    local host="${endpoint%%:*}"
    local port="${endpoint##*:}"
...
append_unity_runtime_args() {
    ...
    local job_worker_count
    job_worker_count=$(get_unity_job_worker_count)
    if [[ "$job_worker_count" =~ ^[0-9]+$ ]] && [ "$job_worker_count" -gt 0 ]; then
        UNITY_RUNTIME_ARGS_RESULT+=("-job-worker-count" "$job_worker_count")
    else
        log "⚠️  Invalid UNITY_JOB_WORKER_COUNT='$job_worker_count', skipping custom Job Worker Count"
    fi

    local gc_helper_count
    gc_helper_count=$(get_unity_gc_helper_count)
    if [[ "$gc_helper_count" =~ ^[0-9]+$ ]] && [ "$gc_helper_count" -gt 0 ]; then
        UNITY_RUNTIME_ARGS_RESULT+=("-gc-helper-count" "$gc_helper_count")
    else
        log "⚠️  Invalid UNITY_GC_HELPER_COUNT='$gc_helper_count', skipping custom GC Helper Count"
    fi

    local accelerator_enabled="${UNITY_ACCELERATOR_ENABLED:-true}"
    if [ "$accelerator_enabled" != "true" ]; then
        log "Unity Accelerator disabled via UNITY_ACCELERATOR_ENABLED=$accelerator_enabled"
        return 0
    fi

    local accelerator_endpoint="${UNITY_ACCELERATOR_ENDPOINT:-127.0.0.1:10080}"
    if ! is_unity_accelerator_available "$accelerator_endpoint"; then
        log "Unity Accelerator not reachable at $accelerator_endpoint, continuing without cache server"
        return 0
    fi

    local namespace_prefix="${UNITY_ACCELERATOR_NAMESPACE_PREFIX:-${PROJECT_NAME}_${UNITY_VERSION}}"
    namespace_prefix=$(echo "$namespace_prefix" | tr ' /' '__')

    UNITY_RUNTIME_ARGS_RESULT+=(
        "-EnableCacheServer"
        "-cacheServerEndpoint" "$accelerator_endpoint"
        "-cacheServerNamespacePrefix" "$namespace_prefix"
        "-cacheServerEnableDownload" "true"
        "-cacheServerEnableUpload" "true"
        "-cacheServerWaitForConnection" "2000"
        "-cacheServerWaitForUploadCompletion"
    )

    log "Using Unity runtime workers: logical CPU=$cpu_count, job-worker-count=$job_worker_count, gc-helper-count=$gc_helper_count"
    log "Unity Accelerator enabled at $accelerator_endpoint with namespace $namespace_prefix"
}

# Auto-clean .utmp CMake cache if it was generated by a different Unity version.
# This prevents [CXX1410] 'ninja' not found errors after upgrading Unity.
cleanup_utmp_if_version_mismatch() {
    local utmp_dir="$PROJECT_PATH/.utmp"
    if [ ! -d "$utmp_dir" ]; then
        return
    fi

    # Find a cached build json to extract which Unity version created it
    local sample_json
    sample_json=$(find "$utmp_dir" -name "android_gradle_build.json" 2>/dev/null | head -1)
    if [ -z "$sample_json" ]; then
        return
    fi

    # Extract Unity version from the Editor path embedded inside the json
    local cached_version
    cached_version=$(grep -o 'Editor/[0-9][^/]*/[^"]*' "$sample_json" | head -1 | sed 's|Editor/||;s|/.*||')
    if [ -z "$cached_version" ]; then
        return
    fi

    if [ "$cached_version" != "$UNITY_VERSION" ]; then
        log "⚠️  .utmp CMake cache was built with Unity $cached_version but current version is $UNITY_VERSION"
        rm -rf "$utmp_dir"
        log "  ✓ Cleared stale .utmp cache (version mismatch)"
    fi
}
...
check_boarddoctor() {
    ...
    if [ -f "$tracker_file" ]; then
        ...
        # Auto-run in release mode; prompt in interactive mode; skip in CI/CD
        if [ -n "${BOARDIBLE_RELEASE:-}" ]; then
            log "Release mode: auto-running BoardDoctor..."
        ...
        # Auto-run in release mode; prompt in interactive mode; warn in CI/CD
        if [ -n "${BOARDIBLE_RELEASE:-}" ]; then
            log "Release mode: running BoardDoctor (first time for this config)..."
            run_boarddoctor "$profile"
        elif [ -t 0 ]; then
            echo ""
            echo "═══════════════════════════════════════════════════"
...
run_boarddoctor() {
    ...
    if bash "$boarddoctor_script" "$profile"; then
        ...
        echo ""
        if [ -n "${BOARDIBLE_RELEASE:-}" ]; then
            log "Release mode: aborting build due to BoardDoctor failure"
            exit 1
        fi
...
build_android() {
    ...
    # Run environment verification (licenses, SDK/NDK integration, properties fix)
    if [ -f "$SCRIPT_DIR/verifyAndroidEnv.sh" ]; then
        log "Running Android environment verification..."
        bash "$SCRIPT_DIR/verifyAndroidEnv.sh" || log "Android environment verification had warnings (non-fatal, continuing build)"
    fi
...
main() {
    ...
    if [ "$CLEAN_CACHE" = true ]; then
        ...
        # Clean .utmp CMake/NDK intermediate cache
        if [ -d "$PROJECT_PATH/.utmp" ]; then
            rm -rf "$PROJECT_PATH/.utmp"
            log "  ✓ Cleared .utmp (NDK/CMake cache)"
        fi
